...
def _movie_header(mode: str, movie: str, source: str, streams: list[Stream]) -> list[str]:
    return [
        "#!/bin/bash",
        f"# Generated by batchbrake ({mode} mode)",
        f"# Movie  : {movie}",
...
def _encode_flags(quality: int, preset: str, force_crop: bool,
    ...
    if animation:
        lines.append(f"    --encoder-tune animation \\")
    if a_tracks:
        lines.append(f"    -a {a_tracks} --aencoder {a_enc} --audio-fallback ac3 \\")
    if s_tracks:
...
def generate_movie_script(
    ...
    a_tracks, a_enc = _audio_flags(streams, audio_tracks)
    s_tracks, s_default = _sub_flags(streams, None, sub_tracks)
    flags = _encode_flags(quality, preset, force_crop,
                          a_tracks, a_enc, s_tracks, s_default,
                          decomb, animation, align_av)

    lines = _movie_header("movie", movie, input_path, streams) + [
        "",
        f'movie="{movie}"',
        f'output_dir="{output_dir}"',
        "",
        'mkdir -p "$output_dir/$movie"',
        "",
        f"# {movie}",
        (
            f"{hb_cmd} \\\n"
            f'    -i "{input_path}" \\\n'
            f'    -o "$output_dir/$movie/$movie.mkv" \\\n'
            f"{flags}"
        ),
        "",
    ]
